...
@testset "Test 1D FDTD solvers" begin
    ...
    Δt = 0.8*Δx
    f₀ = Dict(
        "ey"=> [sin(2π*i*Δx) for i = 0:100],
        "ez"=> [0.0 for i = 0:100],
        "hy"=> [0.0 for i = 0:99],
        "hz"=> [sin(2π*((i+0.5)*Δx-0.5*Δt)) for i = 0:99]
    )

    f₁ = Dict(
        "ey"=> [sin(2π*(i*Δx-10*Δt)) for i = 0:100],
        "ez"=> [0.0 for i = 0:100],
        "hy"=> [0.0 for i = 0:99],
        "hz"=> [sin(2π*((i+0.5)*Δx-10.5*Δt)) for i = 0:99]
    )
    ...
    advance_fields! = FDTD1D.make_advance_fields(length(f₀["hz"]), Δt/Δx, pulse_shape, Δt, Δx, 0)

    for t in 1:10
        advance_fields!(f, 0, :Nothing)
    end
