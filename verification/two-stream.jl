...
# without the note explaining what bounds it -- then a window of about [22.9,
# 24.15], between the slowest fit completing and the fastest run diverging; the
# runs now stop at the velocity Courant limit instead. A bare number with nothing
# saying why is worse than the duplication it came from. Shared for the reason
# `wakefield` is shared: the script and the test that asserts its claims run the
# same setup -- so a growth curve below ends where its run reached the limit.
